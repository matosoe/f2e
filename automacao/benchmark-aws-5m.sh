...
# One physical SQS message contains exactly one logical envelope. SendMessageBatch
# may still transport ten independent messages per API call.
...
run_case() {
  ...
  output_messages="$(wait_queue_count "$output_queue_url" "$records_published")"
  ...
  jq -n \
    ...
    --argjson chunks "$case_records_per_chunk" --argjson workers "$worker_concurrency" --argjson publishConcurrency "$publish_concurrency" \
    ...
    '{label:$label,jobId:$jobId,status:$status,objectKey:$objectKey,startedAt:$startedAt,endedAt:$endedAt,input:{records:$records,recordLengthBytes:$recordLength,sizeBytes:$size,sha256:$sha256},parameters:{recordsPerChunk:$chunks,workerConcurrency:$workers,publishConcurrencyPerWorker:$publishConcurrency,outputMode:"single",envelopesPerMessage:1,sqsApiBatchSize:10},durationsMs:{generation:$generation,upload:$upload,processing:$processing},result:{expectedChunks:$expected,completedChunks:$completed,recordsRead:$read,recordsPublished:$published,messagesPublished:$messages,approximateOutputQueueMessages:$queueMessages,intakeDlq:$intakeDlq,chunkDlq:$chunkDlq,recordsRejected:$rejected,recordsIgnored:$ignored,recordsPerSecond:$throughput,inputMiBPerSecond:$mibps},resources:{cpu:$cpu,memory:$memory,rawLogs:"cloudwatch-logs.json"}}' > "$case_dir/report.json"
  cat "$case_dir/report.json"

  if [[ "$status" != "COMPLETED" || "$records_read" -ne "$records" || "$records_published" -ne "$records" || "$messages_published" -ne "$records" || "$output_messages" -ne "$records" || "$records_rejected" -ne 0 || "$records_ignored" -ne 0 || "$completed_chunks" -ne "$expected_chunks" || "$intake_dlq" -ne 0 || "$chunk_dlq" -ne 0 ]]; then
    echo "Caso $label falhou na validação; 5M não será iniciado." >&2
    return 1
  fi
...
run_case 5000000 5m "$full_records_per_chunk"

jq -n \
  --arg runId "$run_id" --arg account "$account" --arg region "$region" --arg worker "$worker_name" \
  --argjson provisionMs "$provision_ms" --slurpfile smoke "$result_dir/10k/report.json" --slurpfile full "$result_dir/5m/report.json" \
  '{runId:$runId,aws:{accountId:$account,region:$region},workerFunction:$worker,provisionMs:$provisionMs,cases:{"10k":$smoke[0],"5m":$full[0]}}' > "$result_dir/report.json"
