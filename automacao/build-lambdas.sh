...
build="$root/.build"
mkdir -p "$build/organizer" "$build/worker" "$build/completion-publisher"
...
GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-buildid=' -o "$build/completion-publisher/bootstrap" ./cmd/completion-publisher
