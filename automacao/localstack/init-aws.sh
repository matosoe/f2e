...
mkq file-intake-dlq; mkq chunk-jobs-dlq; mkq completion-events-dlq
...
completion_dlq="$(qurl completion-events-dlq)"
completion_dlq_arn="$(awslocal sqs get-queue-attributes --queue-url "$completion_dlq" --attribute-names QueueArn --query 'Attributes.QueueArn' --output text)"
awslocal sqs create-queue --queue-name completion-events --attributes "{\"ReceiveMessageWaitTimeSeconds\":\"1\",\"VisibilityTimeout\":\"60\",\"RedrivePolicy\":\"{\\\"deadLetterTargetArn\\\":\\\"$completion_dlq_arn\\\",\\\"maxReceiveCount\\\":\\\"1\\\"}\"}" >/dev/null
...
global_limits='{"maxFileBytes":10737418240,"maxChunkBytes":67108864,"maxEventBytes":1044480,"maxBatchSize":10,"inputTypes":{"text":{"maxFileBytes":10737418240,"maxRecordBytes":1040384},"json":{"maxFileBytes":10737418240,"maxRecordBytes":1040384},"multi-line":{"maxFileBytes":10737418240,"maxRecordBytes":1040384}}}'
...
for name in organizer worker; do
  ...
  awslocal lambda update-function-configuration --function-name "f2e-$name" --memory-size 1024 --environment "$env" >/dev/null
done
for name in organizer worker; do
  awslocal lambda wait function-updated-v2 --function-name "f2e-$name"
done
...
for pair in "file-intake organizer 1 4" "chunk-jobs worker 1 12"; do
  set -- $pair; arn="$(awslocal sqs get-queue-attributes --queue-url "$(qurl "$1")" --attribute-names QueueArn --query 'Attributes.QueueArn' --output text)"; uuid="$(awslocal lambda list-event-source-mappings --function-name "f2e-$2" --event-source-arn "$arn" --query 'EventSourceMappings[0].UUID' --output text)"; if [[ "$uuid" == None ]]; then awslocal lambda create-event-source-mapping --function-name "f2e-$2" --event-source-arn "$arn" --batch-size "$3" --scaling-config "MaximumConcurrency=$4" --function-response-types ReportBatchItemFailures >/dev/null; else awslocal lambda update-event-source-mapping --uuid "$uuid" --batch-size "$3" --scaling-config "MaximumConcurrency=$4" >/dev/null; fi
done
