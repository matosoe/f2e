...
global_limits='{"maxFileBytes":10737418240,"maxChunkBytes":67108864,"maxEventBytes":262144,"maxBatchSize":10,"maxJsonArraySearchBytes":16777216,"inputTypes":{"fixed-width":{"maxFileBytes":10737418240,"maxRecordBytes":258048},"text":{"maxFileBytes":10737418240,"maxRecordBytes":258048},"jsonl":{"maxFileBytes":10737418240,"maxRecordBytes":258048},"ndjson":{"maxFileBytes":10737418240,"maxRecordBytes":258048},"csv":{"maxFileBytes":67108864,"maxRecordBytes":258048},"json":{"maxFileBytes":10737418240,"maxRecordBytes":258048},"binary":{"maxFileBytes":193536,"maxRecordBytes":193536},"multi-line":{"maxFileBytes":10737418240,"maxRecordBytes":258048}}}'
...
for spec in \
  'example-fixed-width|fixed-width' 'example-text|text' 'example-jsonl|jsonl' \
  'example-ndjson|ndjson' 'example-csv|csv' 'example-binary|binary' \
  'example-json|json' 'example-multi-line|multi-line'; do
  prefix="${spec%%|*}"; data_type="${spec#*|}"
  extra=''
  max_record_length=0
  max_file_bytes=10737418240
  max_chunk_bytes=67108864
  [[ "$data_type" == text || "$data_type" == jsonl || "$data_type" == ndjson ]] && max_record_length=65536
  [[ "$data_type" == csv ]] && max_file_bytes=67108864
  if [[ "$data_type" == binary ]]; then max_file_bytes=193536; max_chunk_bytes=193536; fi
  ...
  value="{\"bucket\":\"f2e-input\",\"prefix\":\"$prefix/\",\"dataType\":\"$data_type\",\"recordLengthBytes\":100,\"recordsPerChunk\":1000,\"batchSize\":10,\"maxEventBytes\":262144,\"maxFileBytes\":$max_file_bytes,\"maxChunkBytes\":$max_chunk_bytes,\"jsonArraySearchBytes\":1048576,\"maxRecordLengthBytes\":$max_record_length,\"eventSchemaId\":\"f2e-record\",\"eventSchemaVersion\":\"1\",\"eventFormat\":\"json\",\"options\":{\"bypassJsonValidation\":false}$extra}"
