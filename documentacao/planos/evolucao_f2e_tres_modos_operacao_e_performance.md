...
| T06 | 2026-09-05 | Concluído | `messages.go`: `ConfigurationSnapshot` com `ConfigHash`, `ParameterName`, `ParameterVersion`, `Responsible`, `LoadedAt`, `GlobalLimitsParameter`, `GlobalLimitsVersion`; `Responsible` adicionado a `PrefixConfiguration`; `ConfigSnapshot` adicionado a `ChunkJob` e `JobPlan`; `ports.go`: `ResolvePrefixConfiguration` retorna `(PrefixConfiguration, ConfigurationSnapshot, error)`, `LoadGlobalLimits` retorna `(GlobalLimits, int64, string, error)`; `prefix_configuration.go`: snapshot capturado com hash SHA-256, nome e versão do parâmetro SSM, `responsible` e `loadedAt`; `LoadGlobalLimits` retorna versão e nome do parâmetro; `organizer/main.go`: snapshot enriquecido com proveniência dos limites globais e propagado para `ChunkJob.ConfigSnapshot`; `organizer/service.go`: `Publish` propaga `ConfigSnapshot` para `JobPlan`; `ledger.go`: `Plan` persiste `configSnapshot` no item DynamoDB do job; `prefix_configuration_test.go`: testes atualizados para verificar snapshot, hash determinístico, versão e responsible; worker já recebe snapshot via `ChunkJob.ConfigSnapshot` sem consultar SSM; `go test ./internal/...`, `go vet`, lambdas e `terraform validate` passam | — |
